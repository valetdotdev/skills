...
The channel prompt file scopes each invocation, but the agent's `SOUL.md` should reinforce the general principle so it applies across all channel prompt files:
...
- When the user asks to set up an agent, guide them through the full workflow (create, connectors, channel prompt files, channels, deploy).
