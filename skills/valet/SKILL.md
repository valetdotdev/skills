...
| `valet logs` | Stream logs from a deployed agent (last 100 lines then live tail; use `-n` to change, `-n 0` for live-only) | `valet help logs` |
...
valet logs --agent my-agent        # Stream last 100 lines then tail live
valet logs --agent my-agent -n 0   # Live-only, skip history
