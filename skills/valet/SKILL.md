...
- **Default org**: A persistent default organization that auto-sets when creating or joining an org. Commands that accept `--org` fall back to the default org when the flag is omitted. Use `valet orgs default` to view, set, or clear it.
...
Output is grouped: personal agents appear first under `== personal`, followed by org groups in alphabetical order.
...
Output is grouped: personal connectors appear first under `== personal`, followed by org groups in alphabetical order.
...
The default org (if set) is marked with `(default)`.
...
Permanently removes the org. Cannot be undone. If this org was your default, the default is automatically cleared.
...
### Manage the default org

Show, set, or clear the persistent default org used by commands that accept `--org`:

```
valet orgs default              # Show the current default org
valet orgs default <name>       # Set a new default org
valet orgs default --clear      # Clear the default org
```

When a default org is set, `agents create`, `connectors create`, and other commands will use it automatically unless `--org` or `--personal` is specified.
...
If this org was your default, the default is automatically cleared.
...
- When the user is working within an org, pass `--org <org-name>` to agent, connector, and secrets commands. If the user has set a default org (`valet orgs default`), commands will use it automatically without needing `--org`.
- Use `--personal` when the user explicitly wants to create an agent or connector in their personal workspace despite having a default org set.
