...
- **Default org**: A persistent CLI setting that automatically scopes `agents create` and `connectors create` to the configured org. Set automatically when creating or joining an org; cleared when leaving or destroying the matching org.
...
Name is optional; the server generates one if omitted. Use `--org` to create within an organization. Use `--personal` to create in your personal workspace even when a default org is configured.
...
Output is grouped: personal agents appear under `== personal`, followed by org groups in alphabetical order (e.g., `== my-org`).
...
**Important**: `--args` takes comma-separated values, not space-separated. Multiple `--env` flags for multiple environment variables. Use `--personal` to create in your personal workspace even when a default org is configured.
...
List output is grouped: personal connectors appear under `== personal`, followed by org groups in alphabetical order.
...
The current default org (if set) is marked with `(default)` in the output.
...
- With no arguments: prints the current default org (or "No default org." if unset)
- With `<name>`: sets the named org as the default
- With `--clear`: clears the default org

When a default org is set, `valet agents create` and `valet connectors create` automatically scope new resources to that org. Use `--personal` on those commands to override and create in your personal workspace instead.
...
Permanently removes the org. Cannot be undone. Clears the default org if it matches.
...
Clears the default org if it matches.
...
`--timeout` defaults to 10 minutes. Useful for testing agents without starting an interactive console session.
...
- If a default org is configured (`valet orgs default`), `agents create` and `connectors create` will scope to that org automatically. Use `--personal` to override.
